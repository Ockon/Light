...
class Parser:
  ...
  def statement(self):
    ...
    if self.current_tok.matches(TT_KEYWORD, 'EXIT'):
      print('Exiting')
      exit()
    ...
    if self.current_tok.matches(TT_KEYWORD, 'DOCUMENTATION'):
      print('')
  ...
  def expr(self):
    ...
    if res.error:
      return res.failure(InvalidSyntaxError(
        self.current_tok.pos_start, self.current_tok.pos_end,
        "Expected 'VAR', 'IF', 'FOR', 'WHILE', 'POSSIBLE', 'FUN', int, float, identifier, '+', '-', '(', '[' or 'NOT'"
      ))
